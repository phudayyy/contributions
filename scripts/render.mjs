...
const openNow = records.filter((r) => r.status === 'open').sort(byDateDesc)
...
if (openNow.length === 0) {
  ...
  inflight.push('| Project | What | Issue | Pull request | Opened |')
  inflight.push('|---|---|---|---|---|')
  for (const r of openNow) {
    const n = (u) => (u ? `[#${u.split('/').pop()}](${u})` : '—')
    inflight.push(
      `| [${r.project.repo}](projects/${slug(r.project.repo)}.md) | ${esc(r.title)} | ` +
      `${n(r.links?.issue)} | ${n(r.links?.pr)} | ${r.dates?.opened ?? '—'} |`
    )
  }
  inflight.push('')
}
...
inflight.push('---')
inflight.push('')
inflight.push('<sub>Generated by `scripts/render.mjs` from `data/contributions.json`. Do not edit by hand.</sub>')
inflight.push('')
